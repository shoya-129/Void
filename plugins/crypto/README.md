...
npx void build
```
This compiles the Rust Cargo workspace source code to WebAssembly and places the final NPM-ready package structure inside the `@void/void-crypto` subdirectory (as configured in `void.json`).

### Step 3: Link the Plugin to Your Application
Navigate back to your Node.js application directory and run:
```bash
npx void add <path-to-this-plugin>/@void/void-crypto
```
*Example (if your app is in `examples/express-void`):*
```bash
npx void add ../../plugins/crypto/@void/void-crypto
```
